...
#define tassert(a)   _tassert_impl((a), __FILE__, __LINE__)

static void
_tassert_impl(bool equal, const char * file, int line)
{
    if (!equal) {
        test_failed();
        terror( "ERROR: assert failed: %s:%d\n", file, line);
        exit(1);
    }
    else {
        test_succeeded();
